...
function transform(time, nwkAddr, fPort, payload) {
  ...
  if (fPort === 1 && payloadBytes.length >= 2) {
    ...
    if ((header & 0x80) === 0x80) {
      ...
      if (com === 0 && payloadBytes.length >= 6) {
        return [{
          ...
          outlet: WIND_SPEED, data: {
            "time": time, "nwkAddr": nwkAddr, "windSpeed": decodeFloat(payloadBytes, 4)
          }
        }];
      } else if (com === 1 && payloadBytes.length >= 6) {
        return [{
          ...
          outlet: RELATIVE_HUMIDITY, data: {
            "time": time, "nwkAddr": nwkAddr, "relativeHumidity": decodeFloat(payloadBytes, 4)
          }
        }];
      } else {
        return [];
...
function test() {
  var tests = {
    "wind reading": [
      ...
      [{ outlet: WIND_DIRECTION, data: { time: "2020-02-06T23:44:04.539Z", nwkAddr: 65959, windDirection: 342 } }, { outlet: WIND_SPEED, data: { time: "2020-02-06T23:44:04.539Z", nwkAddr: 65959, windSpeed: 2.2168541705618606e-41 } }]
    ],
    "air reading": [
      transform("2020-02-06T23:44:04.539Z", 65959, 1, "E4FBf4EGQidwpA=="),
      [{ outlet: AIR_TEMPERATURE, data: { time: "2020-02-06T23:44:04.539Z", nwkAddr: 65959, airTemperature: 15.968999862670898 } }, { outlet: RELATIVE_HUMIDITY, data: { time: "2020-02-06T23:44:04.539Z", nwkAddr: 65959, relativeHumidity: -2.4659374949328326e-38 } }]
    ]
  };
